...
use App\Shop\Products\Exceptions\ProductOutOfStockException;
...
class OrderRepository extends BaseRepository implements OrderRepositoryInterface
{
    ...
    public function associateProduct(Product $product, int $quantity = 1, array $data = [])
    {
        ...
        // Atomic conditional decrement: the WHERE clause is evaluated by the DB
        // together with the UPDATE, so concurrent checkouts can't both succeed
        // in decrementing below zero (no separate read-then-write race window).
        ...
        if (!$decremented) {
            throw new ProductOutOfStockException(
                "Not enough stock for product \"{$product->name}\". Requested: {$quantity}."
            );
        }
