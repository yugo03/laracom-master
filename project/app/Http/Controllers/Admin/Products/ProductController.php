...
class ProductController extends Controller
{
    ...
    public function update(UpdateProductRequest $request, int $id)
    {
        ...
        if ($request->filled('appraiser_name') || $request->filled('grade') || $request->filled('serial_number')
            || $request->filled('appraised_at') || $request->filled('certificate_notes') || $request->hasFile('certificate_file')
        ) {
            $certificateData = $request->only(['appraiser_name', 'grade', 'serial_number', 'appraised_at']);
            $certificateData['notes'] = $request->input('certificate_notes');

            if ($request->hasFile('certificate_file')) {
                $certificateData['file'] = $this->uploadOne($request->file('certificate_file'), 'certificates');
            }

            $productRepo->saveCertificate($certificateData);
        }
